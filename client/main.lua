      ...
      if IsControlPressed(0, Keys['E']) then
        if CurrentAction == 'ambulance_duty' then
          ...
          if PlayerData.job.name == 'ambulance' then
            sendNotification(_U('offduty'), 2)
            Wait(1000)
          else
            sendNotification(_U('onduty'), 1)
            Wait(1000)
          end
        else
          sendNotification(_U('notamb'), 3)
          Wait(1000)
        end
      end
...
function sendNotification(message, messageType)
	TriggerEvent('notification', message, messageType)
end